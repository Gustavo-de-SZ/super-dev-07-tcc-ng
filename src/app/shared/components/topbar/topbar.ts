          ...
          @if (showChatDropdown) {
            <div class="tcc-popover tcc-chat-popover" (click)="$event.stopPropagation()">
              <div class="tcc-popover-header">
                <div class="header-title-box">
                  ...
                  <div>
                    ...
                    <p class="popover-subtitle">Chats diretos com os seus clientes</p>
                  </div>
                </div>
                @if (chamadosAtivosCount > 0) {
              ...
              <div class="tcc-popover-body">
                @if (carregandoChamados) {
                  ...
                  <div class="empty-state">
                    ...
                    <p class="empty-title">Nenhum atendimento ativo</p>
                    <p class="empty-desc">Quando você aceitar ou assumir um chamado, a conversa em tempo real aparecerá aqui.</p>
                    <a routerLink="/painel/chamados" (click)="showChatDropdown = false" class="empty-action-btn">
                    ...
                    @for (chamado of chamados; track (chamado.id || $index)) {
                      <div class="chat-item" (click)="abrirChat(chamado.id, $event)">
                        ...
                        <div class="chat-item-content">
                          ...
                          <p class="ticket-title">{{ chamado.titulo || chamado.descricao || 'Atendimento técnico' }}</p>
                          <div class="ticket-meta">
                            <span class="client-name">
                              <i class="pi pi-user text-xs"></i>
                              {{ chamado.cliente_nome || chamado.cliente?.nome || 'Cliente' }}
                            </span>
              ...
              <div class="tcc-popover-footer">
                <a routerLink="/painel/chamados" (click)="showChatDropdown = false" class="footer-link">
              ...
              <div class="tcc-popover-header">
                <div class="header-title-box">
                  ...
                  <div>
                    ...
                    <p class="popover-subtitle">Alertas e atualizações da plataforma</p>
                  </div>
                </div>
                @if (naoLidasCount > 0) {
...
export class TopbarTecnico implements OnInit, OnDestroy {
  ...
  private setupUserSubscription(): void {
    this.authService.user$.subscribe((user: any) => {
      if (user) {
        this.usuario.nome = (user.name && !user.name.includes('@')) ? user.name : (user.given_name || user.nickname || 'Usuário');
  ...
  loadChamados() {
    this.solicitacaoService.getMinhas().subscribe({
      next: (lista) => {
        const todos = lista || [];
        const ativos = todos.filter((c: any) => {
  ...
  abrirChat(chamadoId: number | string | undefined, event: Event) {
    ...
    this.showChatDropdown = false;
    if (chamadoId) {
      this.router.navigate(['/painel/chat', chamadoId]);
    }
  }
